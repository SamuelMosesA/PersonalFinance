...
def _get_postgres_conn_str(postgres_conf: dict) -> str:
    postgres_conn_str = "postgresql://{username}:{password}@{ipaddress}:{port}/{dbname}".format(
...
def get_config(config_file: Path) -> Config:
    ...
    return Config(
        ...
        debit_stmt_input_dir=Path(parsed_yaml["statement_services"]["debit_stmt_input_dir"]),
        credit_card_stmt_input_dir=Path(parsed_yaml["statement_services"]["credit_card_stmt_input_dir"])
    )